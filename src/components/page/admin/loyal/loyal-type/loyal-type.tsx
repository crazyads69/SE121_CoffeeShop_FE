...
interface LoyalTypeProps {
    selectedLoyalType: string;
    setSelectedLoyalType: (selectedLoyalType: string) => void;
}

export default function LoyalType({ selectedLoyalType, setSelectedLoyalType }: LoyalTypeProps) {
    return (
            ...
            <div className="inline-flex items-center">
                ...
                >
                    <input
                        id="direct-type"
                        type="checkbox"
                        checked={selectedLoyalType === "direct"}
                        onChange={() => {
                            if (selectedLoyalType === "direct") {
                                setSelectedLoyalType("");
                            } else {
                                setSelectedLoyalType("direct");
                            }
                        }}
                        onClick={() => {
                            if (selectedLoyalType === "direct") {
            ...
            <div className="mt-[0.25rem] inline-flex items-center">
                ...
                >
                    <input
                        id="percent-type"
                        type="checkbox"
                        checked={selectedLoyalType === "percent"}
                        onChange={() => {
                            if (selectedLoyalType === "percent") {
                                setSelectedLoyalType("");
                            } else {
                                setSelectedLoyalType("percent");
                            }
                        }}
                        onClick={() => {
                            if (selectedLoyalType === "percent") {
