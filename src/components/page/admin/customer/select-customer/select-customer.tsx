...
import { addSelectedCustomer, removeSelectedCustomer } from "@/redux/slices/customer-slice";

interface SelectCustomerProps {
    customerCode: string;
}

export default function SelectCustomer({ customerCode }: SelectCustomerProps) {
    ...
    useEffect(() => {
        if (selectedCustomerList.includes(customerCode)) {
            setChecked(true);
        } else {
            setChecked(false);
        }
    }, [selectedCustomerList, checked]);
    return (
        <div className="inline-flex items-center">
            <label
                className="relative flex cursor-pointer items-center rounded-full"
                htmlFor={`select-customer-${customerCode}`}
                data-ripple-dark="true"
            >
                <input
                    id={`select-customer-${customerCode}`}
                    type="checkbox"
                    checked={checked}
                    onClick={() => {
                        setChecked(!checked);
                        if (checked) {
                            ...
                            dispatch(removeSelectedCustomer(Number(customerCode)));
                        } else {
                            // Add to selected product list
                            dispatch(addSelectedCustomer(Number(customerCode)));
                        }
                    }}
