...
import DeleteStaff from "@/api/staff/delete-staff";

export interface DeleteStaffItemProps {
    staffID: number;
    setShowDeleteStaffModal: (showDeleteStaffModal: boolean) => void;
    setShowStaffDetail: (showStaffDetail: boolean) => void;
}

export default function DeleteStaffItem({
    staffID,
    setShowDeleteStaffModal,
    setShowStaffDetail,
}: DeleteStaffItemProps) {
    const dispatch = useDispatch();
    const handleDeleteStaff = async () => {
        await DeleteStaff(staffID, setShowDeleteStaffModal, setShowStaffDetail, dispatch);
    };
    return (
        <div
                ...
                <div className="flex h-full items-center justify-center">
                    <div className="relative flex h-[24rem] w-[33.125rem] transform flex-col items-center justify-start overflow-hidden rounded-md bg-white p-[3.12rem] shadow-[0px_5px_12px_0px_rgba(0,0,0,0.10)] transition-all">
                        ...
                        <p className="mt-[0.94rem] text-center font-sans text-[#637381]">
                            Hệ thông sẽ xoá bỏ hoàn toàn nhân viên có mã là
                            <span className="font-sans font-bold text-[#637381]"> {staffID}</span>.
                            <br /> Bạn có chắc chắn muốn xoá ?
                        </p>
