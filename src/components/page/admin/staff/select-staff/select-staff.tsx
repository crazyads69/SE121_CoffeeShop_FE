...
import { addSelectedStaff, removeSelectedStaff } from "@/redux/slices/staff-slice";

export interface SelectStaffProps {
    staffCode: string;
}

export default function SelectStaff({ staffCode }: SelectStaffProps) {
    const dispatch = useDispatch();
    // State for check if product is selected
    const [checked, setChecked] = useState<boolean>(false);
    // State for selected product list
    const selectedStaffs = useSelector((state: RootState) => state.staff.selectedStaff);
    // useEffect for check if product is selected
    useEffect(() => {
        if (selectedStaffs.includes(staffCode)) {
            setChecked(true);
        } else {
            setChecked(false);
        }
    }, [selectedStaffs, checked]);
    return (
        <div className="inline-flex items-center">
            <label
                className="relative flex cursor-pointer items-center rounded-full"
                htmlFor={`select-staff-${staffCode}`}
                data-ripple-dark="true"
            >
                <input
                    id={`select-staff-${staffCode}`}
                    type="checkbox"
                    checked={checked}
                    onClick={() => {
                        setChecked(!checked);
                        if (checked) {
                            ...
                            dispatch(removeSelectedStaff(staffCode));
                        } else {
                            // Add to selected product list
                            dispatch(addSelectedStaff(staffCode));
                        }
                    }}
